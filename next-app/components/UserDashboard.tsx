...
export default function UserDashboard() {
  ...
  const { data: profile, isLoading: profileLoading, error } = useUserProfile(userId || '');
    ...
    <div className="max-w-lg mx-auto bg-gray-900 rounded-lg shadow-lg p-6 mt-6 text-white">
      <div className="flex items-center gap-4 mb-4">
        {profile.avatarUrl ? (
          ...
          <div className="w-16 h-16 rounded-full bg-blue-700 flex items-center justify-center text-2xl">
            👤
          </div>
        )}
        <div>
      ...
      <div className="mb-2">
        ...
        <span className="text-green-400">{profile.reputation}</span>
      </div>
      <div className="mb-2">
      ...
      <div className="mb-2">
        ...
        <span className="text-gray-300">{new Date(profile.joinedAt).toLocaleDateString()}</span>
      </div>
      {profile.email && (
        <div className="mb-2">
