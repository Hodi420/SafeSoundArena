...
class BlockchainService {
  ...
  async executeGameResult(winner, loser, amount) {
    try {
      ...
      logger.info(
        `Game result recorded in block ${receipt.blockNumber}, tx: ${receipt.transactionHash}`
      );
  ...
  async verifyNFTOwnership(collectionAddress, ownerAddress, tokenId) {
    try {
      if (
        !this.web3.utils.isAddress(collectionAddress) ||
        !this.web3.utils.isAddress(ownerAddress)
      ) {
