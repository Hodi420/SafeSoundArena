...
function analyzePerformance(results) {
  ...
  // Calculate averages
  const avgResponseTime = metrics.responseTimes.reduce((sum, rt) => sum + rt.avg, 0) / Math.max(1, metrics.responseTimes.length);
  ...
  const performanceData = [
    ...
    [
      ...
      reqPerSec > 100 ? colors.success('✓ Good') : 
      reqPerSec > 50 ? colors.warning('⚠️ Acceptable') : 
      colors.error('❌ Needs Improvement')
    ],
    [
      'Error Rate', 
      `${errorRate.toFixed(2)}%`,
      errorRate < 1 ? colors.success('✓ Good') : 
      errorRate < 5 ? colors.warning('⚠️ Monitor') : 
      colors.error('❌ Critical')
    ],
    [
      'Avg Response Time', 
      `${avgResponseTime.toFixed(2)}ms`,
      avgResponseTime < 500 ? colors.success('✓ Good') : 
      avgResponseTime < 1000 ? colors.warning('⚠️ Monitor') : 
      colors.error('❌ Critical')
    ]
  ];
