...
export default function Marketplace() {
  ...
  const [sellState, setSellState] = useState<
    Record<string, { quantity: number; price: number; open: boolean }>
  >({});
  ...
  const handleSellSubmit = (item: any) => {
    ...
    setToastError(null);
    sellItem.mutate(
      { itemId: item.id, quantity: state.quantity, price: state.price },
      {
        onSuccess: () => {
      ...
      <div className="bg-gray-900 rounded-lg p-4 border border-yellow-500">
        ...
        <div className="flex flex-col gap-4 mt-4">
          {items.map((item) => (
            <div
              key={item.id}
              className="flex items-center justify-between bg-gray-800 rounded-md p-3 border border-yellow-700 mb-2"
            >
              ...
              >
                {buyItem.isPending ? (
                  ...
                  <svg width="22" height="22" viewBox="0 0 22 22" fill="none" aria-hidden="true">
                    ...
                    <path
                      d="M6 7V5.5A1.5 1.5 0 0 1 7.5 4h7A1.5 1.5 0 0 1 16 5.5V7"
                      stroke="#1b7c1b"
                      strokeWidth="1.3"
                    />
                  </svg>
                ...
                >
                  <label className="text-xs text-gray-300">
                    ...
                    <input
                      ...
                      onChange={(e) =>
                        handleSellChange(item.id, 'quantity', Number(e.target.value))
                      }
