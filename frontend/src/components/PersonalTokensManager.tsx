...
export default function PersonalTokensManager() {
  ...
  const userId =
    typeof window !== 'undefined'
      ? localStorage.getItem('userId') || 'pi-demo-user'
      : 'pi-demo-user';
  ...
  async function onDelete(id: string) {
    setMsg(null);
    await fetch(`${base}/api/user/tokens/${id}?userId=${encodeURIComponent(userId)}`, {
      method: 'DELETE',
    });
    refresh();
  }
    ...
    >
      ...
      >
        <select
          ...
          style={{ padding: 10, borderRadius: 8, border: '1.5px solid #bcd' }}
        >
          {PROVIDERS.map((p) => (
