...
export default function AdminDashboard({ userId }) {
  ...
  useEffect(() => {
    setLoading(true);
    Promise.all([
      fetch('/api/admin/users', { headers: { 'x-admin-token': window.ADMIN_TOKEN || '' } }).then(
        (r) => r.json(),
      ),
  ...
  useEffect(() => {
    const loadTopUsers = (range = 'all') => {
      ...
      fetch(`/api/root/top-users-tasks-stats?range=${range}`)
        ...
        .catch(() => {
          setTopUsersError('שגיאה בטעינת טבלת משתמשים פעילים');
          setTopUsersLoading(false);
        });
    };
  ...
  async function blockUser(uid) {
    await fetch(`/api/admin/users/${uid}/block`, {
      method: 'POST',
  ...
  async function promoteUser(uid) {
    await fetch(`/api/admin/users/${uid}/promote`, {
      method: 'POST',
      headers: { 'x-admin-token': window.ADMIN_TOKEN || '' },
    });
    const updated = await fetch('/api/admin/users', {
  ...
  function toggleAgent() {
    fetch('/api/agent', {
      method: 'POST',
      body: JSON.stringify({ command: 'toggle_agent', confirm: true }),
  ...
  function selfUpdate() {
    fetch('/api/agent/selfupdate', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
    })
      .then((r) => r.json())
      .then((d) => setSelfUpdateMsg(d.message || d.error || 'done'));
  }
  ...
  return (
    <div
      style={{ maxWidth: 800, margin: 'auto', background: '#f5f5f5', padding: 24, borderRadius: 8 }}
    >
      <h2>דשבורד ניהולי</h2>
      <div style={{ marginBottom: 24 }}>
        <h3>סטטיסטיקות</h3>
        <div>
          משתמשים: {counts.users} | משימות: {counts.tasks} | Agents: {counts.agents}
        </div>
        <div style={{ marginTop: 24 }}>
          ...
          <div style={{ marginBottom: 8 }}>
            ...
            <button
              ...
              onClick={() => {
                ...
                a.href = url;
                a.download = 'top-users-tasks.csv';
          ...
          {topUsersLoading ? (
            ...
            <div style={{ color: 'red' }}>{topUsersError}</div>
          ) : (
              ...
              <tbody>
                ...
                {topUsers.map((u, i) => (
                  <tr key={u.user}>
                    ...
                    <td>
                      {u.totalTasks
                        ? ((u.completedTasks / u.totalTasks) * 100).toFixed(1) + '%'
                        : '-'}
                    </td>
                  </tr>
                ))}
    ...
    <div style={{ background: '#222', color: '#fff', borderRadius: 8, padding: 16 }}>
      <div style={{ display: 'flex', alignItems: 'center', marginBottom: 8 }}>
        ...
        <span style={{ fontWeight: 'bold', fontSize: 18 }}>
          Root MCP: {hierarchy.rootMcpId || 'N/A'}
        </span>
      </div>
      <div style={{ marginLeft: 24 }}>
        {hierarchy.miniMcps && hierarchy.miniMcps.length > 0 ? (
          hierarchy.miniMcps.map((mini, i) => <MiniMcpNode key={mini.miniMcpId || i} mini={mini} />)
        ) : (
    ...
    <div style={{ marginBottom: 12 }}>
      ...
      <AnimatePresence>
        {open && (
          ...
          >
            {mini.agents && mini.agents.length > 0 ? (
              mini.agents.map((agent, j) => (
                <div
                  key={agent.agentId || j}
                ...
                >
                  ...
                  <span
                    style={{
                      marginLeft: 8,
                      fontSize: 12,
                      color: agent.status === 'active' ? '#0f0' : '#f00',
                    }}
                  >
                    {agent.status || ''}
                  </span>
