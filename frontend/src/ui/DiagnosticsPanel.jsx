...
export default function DiagnosticsPanel() {
  ...
  return (
    <div
      style={{ padding: '1rem', background: '#fafafa', border: '1px solid #ddd', margin: '1rem 0' }}
    >
      <h3>System Diagnostics</h3>
      <p>
        <strong>Renderer:</strong> {diag.renderer}
      </p>
      {diag.usingIGPU && <p style={{ color: 'orange' }}>Warning: Using integrated GPU</p>}
      <p>
        <strong>Power:</strong> {diag.onBattery ? 'Battery' : 'AC connected'}
      </p>
      {diag.onBattery && <p style={{ color: 'orange' }}>Warning: Running on battery</p>}
      <p>
        <strong>FPS (1s test):</strong> {diag.fps}
      </p>
      {diag.fps < 30 && <p style={{ color: 'red' }}>Warning: Low FPS</p>}
      <p>
        <strong>Memory (GB):</strong> {diag.memGB}
      </p>
      <p>
        <strong>CPU Cores:</strong> {diag.cores}
      </p>
    </div>
  );
}
