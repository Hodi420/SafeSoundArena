...
export default function UserDashboard({ userId }) {
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      .then(([userData, tasksData, notifData]) => {
        ...
        setEmailPrefs(
          userData?.preferences?.notificationEmails || {
            onboarding: true,
            notification: false,
            system: false,
            task: false,
  ...
  return (
    <div
      style={{ maxWidth: 600, margin: 'auto', background: '#fafafa', padding: 24, borderRadius: 8 }}
    >
        ...
        {emailPrefs && (
          ...
          >
            ...
            <label style={{ display: 'block', margin: 4 }}>
              <input
                type="checkbox"
                checked={emailPrefs.notification}
            ...
            <label style={{ display: 'block', margin: 4 }}>
              <input
                type="checkbox"
                checked={emailPrefs.task}
                onChange={(e) => setEmailPrefs((p) => ({ ...p, task: e.target.checked }))}
              />
              קבל מייל על משימות
            </label>
            <button type="submit" disabled={prefsSaving} style={{ marginTop: 8 }}>
              שמור העדפות
            </button>
          ...
          <ul>
            ...
            {tasks.map((task) => (
              <li key={task._id} style={{ marginBottom: 8 }}>
                <b>{task.title}</b> ({task.status}){task.agentId && <> | Agent: {task.agentId}</>}
                {task.status !== 'done' && (
                  <button style={{ marginRight: 8 }} onClick={() => markTaskDone(task._id)}>
                    סמן כבוצע
                  </button>
                )}
                <button style={{ marginRight: 8 }} onClick={() => deleteTask(task._id)}>
                  מחק
                </button>
                {task.description && (
          ...
          <ul>
            ...
            {notifications.map((notif) => (
              <li
                ...
                style={{
                  marginBottom: 8,
                  background: notif.read ? '#e0ffe0' : '#fff7e0',
                  padding: 8,
                  borderRadius: 4,
                }}
              >
                ...
                {!notif.read && (
                  <button style={{ marginRight: 8 }} onClick={() => markNotifRead(notif._id)}>
                    סמן כנקראה
                  </button>
                )}
                <span style={{ float: 'left', fontSize: 10 }}>
