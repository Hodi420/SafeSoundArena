...
app.post('/admin/login', (req, res) => {
  ...
  const token = jwt.sign({ isAdmin: true, timestamp: Date.now() }, JWT_SECRET, { expiresIn: '1h' });
...
app.post('/admin/licenses', adminAuthMiddleware, (req, res) => {
  const { customerName, customerEmail, licenseType, expiresAt, maxUsers, maxInstances, features } =
    req.body;
...
app.get('/admin/licenses', adminAuthMiddleware, (req, res) => {
  ...
  const sanitizedLicenses = licenseData.licenses.map((license) => ({
    ...
    lastVerification: license.verifications?.length
      ? license.verifications[license.verifications.length - 1].timestamp
      : null,
  }));
...
app.get('/admin/stats', adminAuthMiddleware, (req, res) => {
  ...
  const stats = {
    ...
    expiredLicenses: licenseData.licenses.filter(
      (license) => license.expiresAt && new Date(license.expiresAt) < new Date()
    ).length,
    licensesByType: {},
    verificationCount: 0,
    lastUpdated: licenseData.lastUpdated,
