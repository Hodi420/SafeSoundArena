...
async function analyzeHttpMetrics() {
  ...
  // Get error rate
  const errorRate = await queryPrometheus('sum(rate(http_requests_total{status=~"5.."}[5m])) by (service) / sum(rate(http_requests_total[5m])) by (service)');
  
  // Get latency
  const latency = await queryPrometheus('histogram_quantile(0.95, sum(rate(http_request_duration_seconds_bucket[5m])) by (le, service))');
...
async function analyzeResourceUsage() {
  ...
  // CPU usage
  const cpu = await queryPrometheus('100 - (avg by(instance) (rate(node_cpu_seconds_total{mode="idle"}[5m])) * 100');
  
  // Memory usage
  const memory = await queryPrometheus('(node_memory_MemTotal_bytes - node_memory_MemAvailable_bytes) / node_memory_MemTotal_bytes * 100');
  
  // Disk usage
  const disk = await queryPrometheus('(node_filesystem_size_bytes{mountpoint="/"} - node_filesystem_avail_bytes{mountpoint="/"}) / node_filesystem_size_bytes{mountpoint="/"} * 100');
