...
async function main() {
  ...
  const ctr = await hre.ethers.getContractAt(
    'PioneerPoH',
    process.env.POH_ADDR || '0x0000000000000000000000000000000000000000'
  );
  // read root from aggregator out
  const rootHex = fs.readFileSync(__dirname + '/../aggregator/out/root.txt', 'utf8').trim();
  const manifestCid = fs.existsSync(__dirname + '/../aggregator/out/manifest-cid.txt')
    ? fs.readFileSync(__dirname + '/../aggregator/out/manifest-cid.txt', 'utf8').trim()
    : '';
