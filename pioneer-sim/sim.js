...
const projects = [
  ...
  },
  {
    id: 'brightid',
    name: 'BrightID',
    human_accept_prob: 0.90,
    bot_accept_prob: 0.60,
    cost_per_check: 1.0,
    privacy_risk: 0.2,
  ...
  },
  {
    ...
    cost_per_check: 0.8,
    privacy_risk: 0.15,
    notes: 'Continuous off-chain agent + periodic attestations; aims for low friction and sustained signal.'
  }
];
...
function simulate(project, populations, epochs) {
  ...
  for (let e = 0; e < epochs; e++) {
    ...
    for (let i = 0; i < populations.humans; i++) {
      ...
      if (rand() < p) stats.humans.accepted++; else stats.humans.rejected++;
      stats.total_cost += project.cost_per_check;
    }
    for (let i = 0; i < populations.bots; i++) {
      const p = project.bot_accept_prob;
      if (rand() < p) stats.bots.accepted++; else stats.bots.rejected++;
      stats.total_cost += project.cost_per_check;
    }
  }
  return stats;
...
function summarize(project, stats, populations, epochs) {
  ...
  return {
    ...
    avg_cost_per_agent_per_epoch: stats.total_cost / ((populations.humans + populations.bots) * epochs),
...
function printComparison(res) {
  ...
  for (const r of res.results) {
    console.log(`\n${r.name}`);
    console.log(`  precision: ${(r.precision*100).toFixed(2)}%  recall: ${(r.recall*100).toFixed(2)}%  f1: ${(r.f1*100).toFixed(2)}%`);
    console.log(`  FPR: ${(r.false_positive_rate*100).toFixed(2)}%  FNR: ${(r.false_negative_rate*100).toFixed(2)}%`);
    console.log(`  avg cost/agent/epoch: ${r.avg_cost_per_agent_per_epoch.toFixed(2)}  privacy_risk(0-1): ${r.privacy_risk}`);
...
const botFraction = 0.10; // 10% bots
